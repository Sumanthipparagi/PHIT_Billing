...
class ProductClassController {
    ...
    def index() {
        ...
        try {
            respond productClassService.getAll(params.limit, params.offset, params.query)
        }
        catch (Exception ex) {
    ...
    def show() {
        try {
            String id = params.id
            if (id) {
                respond productClassService.get(id)
            }
        }
    ...
    def getAllByEntityId() {
        try {
            if (params.id) {
                respond productClassService.getAllByEntity(0,0,Long.parseLong(params.id))
            }
        }
        catch (ResourceNotFoundException ex)
        {
    ...
    def save() {
        try {
            ...
            respond productClassService.save(jsonObject)
        }
        catch (ResourceNotFoundException ex)
        {
            System.err.println('Controller :' + controllerName + ', action :' + actionName + ', Ex:' + ex)
     ...
     * @param id
     * @param Stock Activity
     * @return updated Product Class
     */
    def update() {
        try {
            String id = params.id
            JSONObject jsonObject = JSON.parse(request.reader.text) as JSONObject
            respond productClassService.update(jsonObject,id)
        }
        catch (ResourceNotFoundException ex)
        {
            System.err.println('Controller :' + controllerName + ', action :' + actionName + ', Ex:' + ex)
    ...
    /**
     * Gets all Product Class in datatables format
     * @return list of Product Class
     */
    def dataTable() {
        try {
            ...
            respond productClassService.dataTables(paramsJsonObject, start, length)
        }
        catch (ResourceNotFoundException ex)
        {
            System.err.println('Controller :' + controllerName + ', action :' + actionName + ', Ex:' + ex)
