...
class DivisionController {
    ...
    def index() {
        ...
        try {
            respond divisionService.getAll(params.limit, params.offset, params.query)
        }
        catch (Exception ex) {
    ...
    def getAllByEntityId() {
        try {
            String id = params.id
            if (id) {
                respond divisionService.getAllByEntityId(params.limit, params.offset,Long.parseLong(id))
            }
        }
        catch (ResourceNotFoundException ex)
        {
    ...
    def save() {
        try {
            ...
            respond divisionService.save(jsonObject)
        }
        catch (ResourceNotFoundException ex)
        {
            System.err.println('Controller :' + controllerName + ', action :' + actionName + ', Ex:' + ex)
    ...
    def update() {
        try {
            String id = params.id
            JSONObject jsonObject = JSON.parse(request.reader.text) as JSONObject
            respond divisionService.update(jsonObject,id)
        }
        catch (ResourceNotFoundException ex)
        {
            System.err.println('Controller :' + controllerName + ', action :' + actionName + ', Ex:' + ex)
    ...
    def dataTable() {
        try {
            ...
            respond divisionService.dataTables(paramsJsonObject, start, length)
        }
        catch (ResourceNotFoundException ex)
        {
            System.err.println('Controller :' + controllerName + ', action :' + actionName + ', Ex:' + ex)
