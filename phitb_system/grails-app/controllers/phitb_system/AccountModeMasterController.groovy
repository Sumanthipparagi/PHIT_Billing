...
class AccountModeMasterController {
    static responseFormats = ['json', 'xml']
    static allowedMethods = [index: "GET", show: "GET", save: "POST", update: "PUT", delete: "DELETE", dataTable: "GET"]

    AccountModeMasterService accountModeMasterService
    /**
    ...
    def index() {
        ...
        try {
            ...
            respond accountModeMasterService.getAll(params.limit, params.offset, params.query, entityId)
        }
        catch (Exception ex) {
    ...
    def show() {
        try {
            String id = params.id
            if (id) {
                respond accountModeMasterService.get(id)
            }
        }
    ...
    def save() {
        try {
            ...
            respond accountModeMasterService.save(jsonObject)
        }
        catch (ResourceNotFoundException ex)
        {
            System.err.println('Controller :' + controllerName + ', action :' + actionName + ', Ex:' + ex)
    ...
    def update() {
        try {
            String id = params.id
            JSONObject jsonObject = JSON.parse(request.reader.text) as JSONObject
            respond accountModeMasterService.update(jsonObject,id)
        }
        catch (ResourceNotFoundException ex)
        {
            System.err.println('Controller :' + controllerName + ', action :' + actionName + ', Ex:' + ex)
    ...
    def delete() {
        try {
            String id = params.id
            accountModeMasterService.delete(id)
            response.status = 200
        }
        catch (ResourceNotFoundException ex)
        {
    ...
    def dataTable() {
        try {
            ...
            respond accountModeMasterService.dataTables(paramsJsonObject, start, length)
        }
        catch (ResourceNotFoundException ex)
        {
            System.err.println('Controller :' + controllerName + ', action :' + actionName + ', Ex:' + ex)
