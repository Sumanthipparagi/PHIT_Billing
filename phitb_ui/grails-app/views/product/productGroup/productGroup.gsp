...
<head>
    ...
    <title>:: PharmIt :: Product Group</title>
                ...
                <div class="card">
                    ...
                    <div class="header">
                        <button type="button" class="btn btn-round btn-primary m-t-15 addbtn" data-toggle="modal"
                                data-target="#addproductGroupModal"><font style="vertical-align: inherit;"><font
                                style="vertical-align: inherit;">Add Product Group</font></font></button>
                    </div>
                    <div class="body">
...
<g:include view="controls/product/add-product-group.gsp"/>
...
<script>
    ...
    function fridgeTable() {
        productcategorytable = $(".fridgeTable").DataTable({
            ...
            serverSide: true,
            language: {
                searchPlaceholder: "Search Project Group"
            },
            ajax: {
                type: 'GET',
                url: '/phitb_ui.product-group/datatable',
                dataType: 'json',
                dataSrc: function (json) {
                    ...
                    for (var i = 0; i < json.data.length; i++) {
                        ...
                        var editbtn = '<button type="button" data-id="' + json.data[i].id +
                            '" data-groupName="' + json.data[i].groupName + '"' +
                            '" data-groupDescription="' + json.data[i].groupDescription + '"' +
                            '" data-entityId="' + json.data[i].entityId + '"' +
                            '" data-entityType="' + json.data[i].entityTypeId + '"' +
                            '"' +
                            ' class="editbtn btn btn-sm btn-warning  editbtn" data-toggle="modal" data-target="#addproductGroupModal"><i class="material-icons"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">edit</font></font></i></button>'
    ...
    $(".productGroupForm").submit(function (event) {
        ...
        var type = '';
        if (id) {
            url = '/phitb_ui.product-group/update/' + id;
            type = 'POST'
        } else {
            url = '/phitb_ui.product-group';
            type = 'POST'
        }
        ...
        $.ajax({
            ...
            processData: false,
            success: function () {
                swal("Success!", "Product Group Submitted Successfully", "success");
                fridgeTable();
    ...
    $(document).on("click", ".editbtn", function () {
        id = $(this).data('id');
        $(".groupName").val($(this).attr('data-groupName'));
    ...
    $(document).on("click", ".deletebtn", function () {
        id = $(this).data('id');
        $("#myModalLabel").text("Delete Product Group ?");
    ...
    function deleteData() {
        $.ajax({
            type: 'POST',
            url: '/phitb_ui.product-group/delete/' + id,
            dataType: 'json',
            success: function () {
                $('.deleteModal').modal('hide');
                fridgeTable();
                swal("Success!", "Product Group Deleted Successfully", "success");
            }, error: function () {
                swal("Error!", "Something went wrong", "error");
            }
        });
