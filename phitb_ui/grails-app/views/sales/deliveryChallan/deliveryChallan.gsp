...
<head>
    ...
    <title>:: PharmIt ::  Delivery Challan</title>
                                ...
                                %{--<select class="form-control show-tick" id="customerSelect"
                                    ...
                                    <g:each in="${customers}" var="cs">
                                        ...
                                        <g:if test="${cs.id != session.getAttribute("entityId")}">
                                            <option data-state="${cs.stateId}" value="${cs.id}">${cs.entityName}
                ...
                <div class="card">
                    ...
                    <div class="body">
                        ...
                        <div class="row">
                            ...
                            <button id="saveDraftBtn" onclick="saveDeliveryChallan('DRAFT')"
                                    class="btn btn-primary">Save Draft</button>
                            <button id="saveBtn" onclick="saveDeliveryChallan('ACTIVE')"
...
<script>
    ...
    var deliveryChallanId = 0;

    function saveDeliveryChallan(billStatus) {
        $("#saveBtn").prop("disabled", true);
        $("#saveDraftBtn").prop("disabled", true);
        var waitingSwal = Swal.fire({
            title: "Generating Delivery Challan, Please wait!",
            showDenyButton: false,
            showCancelButton: false,
            showConfirmButton: false,
            allowOutsideClick: false
        ...
        var deliveryChallanData = JSON.stringify(hot.getSourceData());
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                deliveryChallanId = data.deliveryChallan.id;
                var datepart = data.deliveryChallan.entryDate.split("T")[0];
                var month = datepart.split("-")[1];
                var year = datepart.split("-")[0];
                var seriesCode = data.series.seriesCode;
                var invoiceNumber = data.deliveryChallan.invoiceNumber;
                ...
                if (billStatus !== "DRAFT") {
                    message = 'Delivery Challan Generated: ' + invoiceNumber;
                } else {
                    message = 'Draft Delivery Challan Generated: ' + data.deliveryChallan.entityId + "/DR/DC/" + month
                        + year + "/"
                        + seriesCode + "/__";
                }
                waitingSwal.close();
    ...
    function printInvoice() {
        if (readOnly) {
            window.open(
                '/delivery-challan/print-delivery-challan?id=' + deliveryChallanId,
                '_blank'
            );
            resetData();
        }
