    ...
    <tr>
        ...
        <td colspan="4" style="vertical-align:top;font-size:8pt;">
            ...
            <ul style="margin: 0;">
                ...
                <li><b class="tab">Invoice No</b>:  <strong><g:if test="${deliveryChallanDetail.billStatus == 'CANCELLED'}"><del>${deliveryChallanDetail.invoiceNumber}</del></g:if><g:else>${deliveryChallanDetail.invoiceNumber}</g:else></strong></li>
...
%{--    </g:if>--}%
    ...
    <g:each var="sp" in="${deliveryChallanProductDetails}" status="i">
        <tr>
            <td>${i + 1}</td>
    ...
    <tr>
        <td colspan="14" style="border: 0"><g:if test="${deliveryChallanDetail.billStatus == 'CANCELLED'}">
            <div id="watermark" class="print-watermark">CANCELLED</div>
        </g:if>
            <g:elseif test="${deliveryChallanDetail.billStatus == 'DRAFT'}">
            ...
            <g:each var="t" in="${termsConditions}" status="i">
                <g:if test="${t?.form?.formType == Constants.DELIVERY_CHALLAN && t?.deleted == false}">
...
<script>
    window.onload = function () {
        ...
        var invDate = new Date('${deliveryChallanDetail.dateCreated}');
        var dueDate = new Date('${deliveryChallanDetail.dueDate}');
        $("#invDate").text(moment(invDate).format('DD-MM-YYYY'));
        $("#dueDate").text(moment(dueDate).format('DD-MM-YYYY'));

        <g:each var="spd" in="${deliveryChallanProductDetails}">
    ...
    var qrText = '${deliveryChallanDetail.invoiceNumber}';
%{--    <g:if test="${irnDetails != null}">--}%
