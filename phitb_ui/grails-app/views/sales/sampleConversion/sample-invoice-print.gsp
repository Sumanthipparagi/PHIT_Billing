...
%{--    </g:if>--}%
    ...
    <g:each var="sp" in="${sampleProductDetails}" status="i">
        <tr>
            <td>${i + 1}</td>
    ...
    <tr>
        <td colspan="14" style="border: 0"><g:if test="${sampleBillDetail.billStatus == 'CANCELLED'}">
            <div id="watermark" class="print-watermark">CANCELLED</div>
        </g:if>
            <g:elseif test="${sampleBillDetail.billStatus == 'DRAFT'}">
...
<script>
    window.onload = function () {
        ...
        var invDate = new Date('${sampleBillDetail.dateCreated}');
        var dueDate = new Date('${sampleBillDetail.dueDate}');
        $("#invDate").text(moment(invDate).format('DD-MM-YYYY'));
        $("#dueDate").text(moment(dueDate).format('DD-MM-YYYY'));

        <g:each var="spd" in="${sampleProductDetails}">
    ...
    var qrText = '${sampleBillDetail.invoiceNumber}';
    <g:if test="${irnDetails != null}">
