...
<div id="wrapper">
    ...
    <table id="prodDetails" class="extended" style="width: 100%;
    padding: 5%;border: 1px solid #000;margin-right:500px;">
        <div class="page-header">
            <thead>
            <tr>
            ...
            <g:each var="key, groupData" in="${groupDetails}" status="i">
                ...
                <g:each var="pd" in="${groupData}" status="j">
                    ...
                    <tr>
                        ...
                        %{--            <td></td>--}%
                        <td>${pd.mrp}</td>
                        <td>${pd.invoiceNumber}</td>
                        <td><g:if test="${pd.reason == 'R'}">SALE RETURN</g:if><g:elseif
                                test="${pd.reason == 'E'}">PRODUCT EXPIRED</g:elseif><g:elseif
                                test="${pd.reason == 'B'}">BREAKAGE</g:elseif><g:elseif
                                test="${pd.reason == 'OA'}">OTHERS (ADD)</g:elseif><g:elseif
                                test="${pd.reason == 'ONE'}">OTHERS (NO EFCT)</g:elseif></td>
                        <td>${pd?.batch?.ptr}</td>
                        <td>${pd.sRate}</td>
                        <td>${pd.sqty}</td>
                        <td>${pd.freeQty}</td>
