 ...
 */
    ...
    def getSaleBillDetailsByPendingIRN(String financialYear, String entityId) {
        JSONObject jsonObject = new JSONObject()
        jsonObject.put("financialYear", financialYear)
        jsonObject.put("entityId", entityId)
        Client client = ClientBuilder.newClient();
        ...
        try {
            Response apiResponse = target
                    .path(new Links().SALE_BILL_PENDING_IRN)
                    .request(MediaType.APPLICATION_JSON_TYPE)
                    .post(Entity.entity(jsonObject.toString(), MediaType.APPLICATION_JSON_TYPE))
            if (apiResponse.status == 200) {
                return new JSONArray(apiResponse.readEntity(String.class))
    ...
    boolean salesProductDeleteCheck(String productId) {
        ...
        try {
            Response apiResponse = target
                    .path(new Links().SALES_PRODUCT_DEL_CHECK)
    ...
    boolean salesBatchDeleteCheck(String productId, String batchNumber) {
        ...
        try {
            Response apiResponse = target
                    .path(new Links().SALES_BATCH_DEL_CHECK)
                    .queryParam("id", productId)
...
//        WebTarget target = client.target(new Links().API_GATEWAY);
    ...
    def updateCanvasImageUrl(JSONObject jsonObject) {
        Client client = ClientBuilder.newClient();
//        WebTarget target = client.target(new Links().API_GATEWAY);
        WebTarget target = client.target("http://localhost:8083");
        try {
            println(jsonObject)
            Response apiResponse = target
                    .path(new Links().SALE_BILL_UPDATE_CANVAS_IMAGE_URL)
                    .request(MediaType.APPLICATION_JSON_TYPE)
                    .post(Entity.entity(jsonObject.toString(), MediaType.APPLICATION_JSON_TYPE))
            println(apiResponse)
            return apiResponse
