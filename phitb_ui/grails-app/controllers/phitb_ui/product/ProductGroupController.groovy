...
class ProductGroupController {
    ...
    def index()
    {
        try
        {
            ...
            render(view: '/phitb_ui/product/productGroup/productGroup',model: [entity     :entity, statelist:statelist,
    ...
    def save()
    {
        ...
        {
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new ProductService().saveProductGroup(jsonObject)
            if (apiResponse?.status == 200)
            {
    ...
    def update()
    {
        try
        {
            println(params)
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new ProductService().putProductGroup(jsonObject)
            if (apiResponse.status == 200)
            {
                JSONObject obj = new JSONObject(apiResponse.readEntity(String.class))
                respond obj, formats: ['json'], status: 200
    ...
    def delete()
    {
        ...
        {
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new ProductService().deleteProductGroup(jsonObject)
            if (apiResponse.status == 200)
            {
    ...
    def show()
    {
        try
        {
            def apiResponse = new ProductService().getProductGroups()
            if (apiResponse?.status == 200)
            {
