...
class ProductScheduleController {
    ...
    def save()
    {
        try
        {
            ...
            def apiResponse = new ProductService().saveProductSchedule(jsonObject)
            if (apiResponse?.status == 200)
            {
    ...
    def update()
    {
        try
        {
            ...
            def apiResponse = new ProductService().putProductSchedule(jsonObject)
            if (apiResponse.status == 200)
            {
                JSONObject obj = new JSONObject(apiResponse.readEntity(String.class))
                respond obj, formats: ['json'], status: 200
    ...
    def delete()
    {
        ...
        {
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new ProductService().deleteProductSchedule(jsonObject)
            if (apiResponse.status == 200)
            {
    ...
    def show()
    {
        try
        {
            def apiResponse = new ProductService().getProductSchedules()
            if (apiResponse?.status == 200)
            {
    ...
    def getByEntity()
    {
        try
        {
            def apiResponse = new ProductService().getProductSchedulesByEntity(session.getAttribute("entityId").toString())
            if (apiResponse?.status == 200)
            {
                JSONArray jsonArray = new JSONArray(apiResponse.readEntity(String.class));
                ArrayList<String> arrayList = new ArrayList<>(jsonArray)
