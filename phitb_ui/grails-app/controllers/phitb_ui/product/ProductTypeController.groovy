...
class ProductTypeController {
    ...
    def index()
    {
        try
        {
            ...
            render(view: '/product/productType/productType',model: [entity     :entity, statelist:statelist,
    ...
    def dataTable()
    {
        try
        {
            ...
            def apiResponse = new ProductService().showProductType(jsonObject)
    ...
    def delete()
    {
        ...
        {
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new ProductService().deleteProductType(jsonObject)
            if (apiResponse.status == 200)
            {
    ...
    def show()
    {
        try
        {
            def apiResponse = new ProductService().getProductTypes()
            if (apiResponse?.status == 200)
            {
    ...
    def getByEntity()
    {
        try
        {
            def apiResponse = new ProductService().getProductTypesByEntity(session.getAttribute("entityId").toString())
            if (apiResponse?.status == 200)
            {
                JSONArray jsonArray = new JSONArray(apiResponse.readEntity(String.class));
                ArrayList<String> arrayList = new ArrayList<>(jsonArray)
