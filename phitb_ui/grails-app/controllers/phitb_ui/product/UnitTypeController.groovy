...
class UnitTypeController {
    ...
    def save()
    {
        ...
        {
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new ProductService().saveUnitType(jsonObject)
            if (apiResponse?.status == 200)
            {
    ...
    def update()
    {
        try
        {
            println(params)
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new ProductService().putUnitType(jsonObject)
            if (apiResponse.status == 200)
            {
                JSONObject obj = new JSONObject(apiResponse.readEntity(String.class))
                respond obj, formats: ['json'], status: 200
    ...
    def delete()
    {
        ...
        {
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new ProductService().deleteUnitType(jsonObject)
            if (apiResponse.status == 200)
            {
    ...
    def getByEntity()
    {
        try
        {
            def apiResponse = new ProductService().getUnitTypeByEntity(session.getAttribute("entityId").toString())
            if (apiResponse?.status == 200)
            {
                JSONArray jsonArray = new JSONArray(apiResponse.readEntity(String.class));
                ArrayList<String> arrayList = new ArrayList<>(jsonArray)
