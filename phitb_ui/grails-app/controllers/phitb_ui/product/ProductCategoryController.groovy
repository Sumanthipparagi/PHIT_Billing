...
class ProductCategoryController {
    ...
    def delete()
    {
        ...
        {
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new ProductService().deleteProductCategory(jsonObject)
            if (apiResponse.status == 200)
            {
    ...
    def show()
    {
        try
        {
            def apiResponse = new ProductService().getProductCategories()
            if (apiResponse?.status == 200)
            {
    ...
    def getByEntity()
    {
        try
        {
            def apiResponse = new ProductService().getProductCategoriesByEntity(session.getAttribute("entityId").toString())
            if (apiResponse?.status == 200)
            {
                JSONArray jsonArray = new JSONArray(apiResponse.readEntity(String.class));
                ArrayList<String> arrayList = new ArrayList<>(jsonArray)
