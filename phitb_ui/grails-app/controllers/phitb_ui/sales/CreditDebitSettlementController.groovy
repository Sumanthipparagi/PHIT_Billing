...
class CreditDebitSettlementController
{
        ...
        try
        {
            ...
            for (JSONObject jsonObject : jsonArray)
            {
                ...
                if (jsonObject.type == 'PI')
                {
                    ...
                    crdb.put('creditFinancialYear', jsonObject.finyear)
                    crdb.put('transType', "CREDIT")
                    def updatePurInBalance = new AccountsService().updatePurchaseBalance(jsonObject)
                    println(updatePurInBalance)
                }

                if (jsonObject.type == 'PR')
                {
                    jsonObject.put('id', jsonObject.billid)
                    jsonObject.put('paidNow', jsonObject.pendamt)
                    jsonObject.put('status', 'NA')
                    ...
                    crdb.put('debitFinancialYear', jsonObject.finyear)
                    crdb.put('transType', "DEBIT")
                    def updatePurRetBalance = new AccountsService().updatePurchaseReturnBalance(jsonObject)
                    println(updatePurRetBalance)
                }
    ...
    def getAllBillDetailsByCustomerId()
    {
        try
        {
            ...
            def purchasebill = new AccountsService().getAllPurchaseBillById(params.id, session.getAttribute("entityId")
                    .toString(), session.getAttribute("financialYear").toString())
            def purchaseRet = new AccountsService().getAllPurchaseReturnById(params.id, session.getAttribute("entityId")
                    .toString(), session.getAttribute("financialYear").toString())
            if (salebill.status == 200 && creditNote.status == 200 && purchasebill.status == 200 && purchaseRet.status == 200)
            {
                ...
                JSONArray purchasebillArray = new JSONArray(purchasebill.readEntity(String.class))
                JSONArray purchaseReturnArray = new JSONArray(purchaseRet.readEntity(String.class))
                jsonArray.add(salearray)
                jsonArray.add(creditNoteArry)
                jsonArray.add(purchasebillArray)
                jsonArray.add(purchaseReturnArray)
