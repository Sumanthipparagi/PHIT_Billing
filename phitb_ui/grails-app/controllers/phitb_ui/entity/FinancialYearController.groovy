...
class FinancialYearController {
    ...
    def index()
    {
        try
        {
            ...
            render(view: '/entity/financialYear/financialYear',model: [entity:entity, entitytype:entitytype,
    ...
    def update()
    {
        try
        {
            println(params)
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new EntityService().putFinancialYear(jsonObject)
            if (apiResponse.status == 200)
            {
                JSONObject obj = new JSONObject(apiResponse.readEntity(String.class))
                respond obj, formats: ['json'], status: 200
    ...
    def delete()
    {
        ...
        {
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new EntityService().deleteFinancialYear(jsonObject)
            if (apiResponse.status == 200)
            {
