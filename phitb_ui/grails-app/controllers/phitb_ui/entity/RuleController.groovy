...
class RuleController {
    ...
    def index()
    {
        try
        {
            ...
            render(view: '/entity/rule/rule',model: [entity:entity, entitytype:entitytype,
    ...
    def save()
    {
        ...
        {
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new EntityService().saveRule(jsonObject)
            if (apiResponse?.status == 200)
            {
    ...
    def update()
    {
        try
        {
            println(params)
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new EntityService().putRule(jsonObject)
            if (apiResponse.status == 200)
            {
                JSONObject obj = new JSONObject(apiResponse.readEntity(String.class))
                respond obj, formats: ['json'], status: 200
    ...
    def delete()
    {
        ...
        {
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new EntityService().deleteRule(jsonObject)
            if (apiResponse.status == 200)
            {
