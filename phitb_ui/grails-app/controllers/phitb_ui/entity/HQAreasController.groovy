...
class HQAreasController {
    ...
    def dataTable()
    {
        try
        {
            ...
            def apiResponse = new EntityService().showHqArea(jsonObject)
    ...
    def save()
    {
        try
        {
            ...
            def apiResponse = new EntityService().saveHqArea(jsonObject)
            if (apiResponse?.status == 200)
            {
    ...
    def update()
    {
        try
        {
            ...
            def apiResponse = new EntityService().putHqArea(jsonObject)
            if (apiResponse.status == 200)
            {
                JSONObject obj = new JSONObject(apiResponse.readEntity(String.class))
                respond obj, formats: ['json'], status: 200
    ...
    def delete()
    {
        ...
        {
            JSONObject jsonObject = new JSONObject(params)
            def apiResponse = new EntityService().deleteHqArea(jsonObject)
            if (apiResponse.status == 200)
            {
