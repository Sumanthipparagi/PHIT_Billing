...
class BillPaymentLogController {
	static responseFormats = ['json', 'xml']
    static allowedMethods = [index: "GET", show: "GET", save: "POST", update: "PUT", delete: "DELETE", dataTable: "GET"]

    BillPaymentLogService billPaymentLogService
    /**
    ...
    def index() {
        ...
        try {
            respond billPaymentLogService.getAll(params.limit, params.offset, params.query)
        }
        catch (Exception ex) {
    ...
    def show() {
        try {
            String id = params.id
            if (id) {
                respond billPaymentLogService.get(id)
            }
        }
    ...
    def save() {
        try {
            ...
            respond billPaymentLogService.save(jsonObject)
        }
        catch (ResourceNotFoundException ex)
        {
            System.err.println('Controller :' + controllerName + ', action :' + actionName + ', Ex:' + ex)
    ...
    def update() {
        try {
            String id = params.id
            JSONObject jsonObject = JSON.parse(request.reader.text) as JSONObject
            respond billPaymentLogService.update(jsonObject,id)
        }
        catch (ResourceNotFoundException ex)
        {
            System.err.println('Controller :' + controllerName + ', action :' + actionName + ', Ex:' + ex)
    ...
    def delete() {
        try {
            String id = params.id
            billPaymentLogService.delete(id)
            response.status = 200
        }
        catch (ResourceNotFoundException ex)
        {
